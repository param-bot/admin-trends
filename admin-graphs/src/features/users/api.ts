...
export async function fetchUserByAccountId(accountId: string): Promise<AdminUser | null> {
  const { data: envelope } = await adminApiClient.get<ApiEnvelope<UsersResponseData>>(
    USERS_PATH,
    {
  ...
  if (!envelope.success) {
    throw new Error(envelope.error?.details ?? envelope.error?.message ?? "Failed to load user")
  }

  return envelope.data.items.find((user) => user.account_id === accountId) ?? null
}
